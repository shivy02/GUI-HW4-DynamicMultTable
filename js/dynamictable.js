...
function addTableTab(values) {
  var label = "Tab: " + values.x1 + "-" + values.x2 + ", " + values.y1 + "-" + values.y2;
  ...
  if (existingTab.length > 0) {
    ...
    $("#tabs ul").append(tabTemplate.replace(/#\{href\}/g, "#" + id).replace(/#\{label\}/g, label));
    $("#tabs").append("<div id='" + id + "' class='table-container'></div>");
    $("#tabs").tabs("refresh");
    $("#tabs").tabs("option", "active", -1); // Activate the last added tab

    // Add the table content to the new tab with the required classes
    $("#" + id).html("<div class='table-responsive'><table class='table table-striped'>" + $(".table-responsive table").html() + "</table></div>");
  }
}
